(function (scope) {
    ...
    function wrapJQueryPromise(deferred) {
        var oldPromise = deferred.promise;

        return function () {
            var promise = oldPromise.apply(this, arguments);
            var oldThen = promise.then;

            promise.then = function () {
                var thenable;
                try {
                    thenable = oldThen.apply(this, arguments);
                } catch (error) {
    ...
    $.Deferred = function () {
        ...
        deferred.resolve = function (resolvedValue) {
            ...
            resolvedDeferred.promise = wrapJQueryPromise(resolvedDeferred);

            return resolvedDeferred;
        };

        deferred.promise = wrapJQueryPromise(deferred);
